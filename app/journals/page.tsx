      ...
      <div className="flex-1 max-w-5xl mx-auto w-full px-6 py-32">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
          <div>
            <h1 className="text-4xl md:text-5xl font-black font-heading text-gray-900 tracking-tight mb-2">
              <span className="text-blue-600">Journals.</span>
            </h1>
            <p className="text-xl text-gray-600">Daily logs, ideas, and personal reflections.</p>
          </div>
          
          <div className="flex items-center gap-4 w-full md:w-auto">
            <form className="relative flex-1 md:w-64" action="/journals" method="GET">
              <input 
                type="text" 
                name="q"
                defaultValue={query}
        ...
        {notes.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {notes.map((note) => (
              <Link href={`/journals/${note.id}`} key={note.id} className="group">
                <article className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 h-full flex flex-col hover:-translate-y-1">
                  ...
                  <div className="mt-auto flex items-center text-sm font-semibold text-gray-900 group-hover:text-blue-600">
                    Read Journal <span className="ml-2 transition-transform group-hover:translate-x-1">→</span>
                  </div>
                </article>
              </Link>
            ))}
